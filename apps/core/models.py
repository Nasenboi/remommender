...
class SongFeatures(models.Model):
    ...
    def to_dict(self, include: List[str] = None) -> dict:
        ...
        return {field: getattr(self, field) for field in include if hasattr(self, field)}
...
class Song(models.Model):
    ...
    features = models.OneToOneField(SongFeatures, on_delete=models.CASCADE, related_name="song")
    genres = models.OneToOneField(SongGenres, on_delete=models.CASCADE, related_name="song")
    ...
    def __str__(self):
        return f"{self.title} by {self.artist}" + (f" from {self.album_name}" if self.album_name else "")
